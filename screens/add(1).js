...
const AddProductScreen = ({onAddSuccess}) => {
  ...
  // Hàm xử lý khi người dùng chọn ảnh từ thư viện
  // const handleChooseImage = () => {
    ...
    //     setImage(response.uri); // Lưu đường dẫn ảnh
    //   }
    // });
  // };
  ...
  const handleAddProduct = () => {
    ...
    axios
      .post('http://192.168.1.14:3000/add-product', productData)
      .then(response => {
        setShowSuccess(true);
    ...
    <View style={styles.notifyContainer}>
      {showSuccess ? (
        <View style={styles.successMessage}>
          <Text>Thêm sản phẩm thành công!</Text>
          <Button 
            title="Đóng"
            onPress={handleReset} 
          />
